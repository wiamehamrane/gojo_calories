...
class _ExerciseEntry {
  ...
  _ExerciseEntry({required this.name, required this.sets, required this.expanded});
}
...
int _estimateCalories(List<_ExerciseEntry> exercises) {
  ...
  final double minutesTotal = totalSets * 2.5; // ~2.5 min per set including rest
...
class _WeightLiftingScreenState extends ConsumerState<WeightLiftingScreen> {
  ...
  void _addSet(int exerciseIdx) {
    setState(() {
      ...
      _exercises[exerciseIdx].sets.add(_SetEntry(reps: lastSet.reps, weight: lastSet.weight, done: false));
    });
  }
  ...
  void _saveWorkout(String lang) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('🏋️ ${Translations.t(lang, 'calories_burned')}: $calories kcal'),
  ...
  Widget build(BuildContext context) {
    ...
    final doneSets = _exercises.fold<int>(0, (s, e) => s + e.sets.where((st) => st.done).length);
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text('Add Exercise', style: const TextStyle(fontWeight: FontWeight.w700)),
  ...
  Widget _buildSummaryStrip(int cal, int total, int done, String lang) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          _StripStat(label: Translations.t(lang, 'calories_burned'), value: '~$cal kcal', icon: LucideIcons.flame),
          Container(width: 1, height: 32, color: Colors.white24),
          _StripStat(label: 'Sets', value: '$done/$total', icon: LucideIcons.check),
          Container(width: 1, height: 32, color: Colors.white24),
          _StripStat(label: 'Exercises', value: '${_exercises.length}', icon: LucideIcons.dumbbell),
        ],
      ),
    );
  }
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(LucideIcons.dumbbell, size: 44, color: AppColors.primary),
            ),
            const SizedBox(height: 20),
            Text(Translations.t(lang, 'weight_lifting'),
...
class _StripStat extends StatelessWidget {
  ...
  const _StripStat({required this.label, required this.value, required this.icon});
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('${entry.sets.length} set${entry.sets.length != 1 ? 's' : ''}',
                            style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                      ],
                    ),
                  ),
                  IconButton(
                    icon: const Icon(LucideIcons.trash2, size: 18, color: AppColors.inactive),
                    onPressed: widget.onRemove,
                  ),
                  AnimatedRotation(
                    turns: entry.expanded ? 0 : -0.25,
                    duration: const Duration(milliseconds: 220),
                    child: const Icon(LucideIcons.chevronDown, size: 20, color: AppColors.inactive),
                  ),
                ],
              ),
            ),
            ...
            child: Column(
              children: [
                ...
                const Padding(
                  ...
                  child: Row(
                    children: [
                      SizedBox(width: 32, child: Text('Set', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.inactive))),
                      Expanded(child: Text('Weight (kg)', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.inactive))),
                      Expanded(child: Text('Reps', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.inactive))),
                      SizedBox(width: 52, child: Text('Done', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.inactive))),
                    ],
                  ),
                ),
                ...
                ...entry.sets.asMap().entries.map((e) {
                  ...
                  return _SetRow(
                    setIndex: si,
                    set: set,
                    onRemove: entry.sets.length > 1 ? () => widget.onRemoveSet(si) : null,
                    onChanged: () {
                      widget.onSetChanged();
                      setState(() {});
                    },
...
class _SetRowState extends State<_SetRow> {
  ...
  void initState() {
    super.initState();
    _weightCtrl = TextEditingController(text: widget.set.weight.toStringAsFixed(1));
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: isDone ? AppColors.primary.withValues(alpha: 0.08) : AppColors.surfaceMuted,
        borderRadius: BorderRadius.circular(12),
        border: isDone ? Border.all(color: AppColors.primary.withValues(alpha: 0.3)) : null,
      ),
              ...
              children: [
                GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                          color: isDone ? AppColors.primary : AppColors.inactive, width: 2),
                ...
                if (widget.onRemove != null) ...[
                  ...
                  GestureDetector(
                    ...
                    child: const Icon(LucideIcons.x, size: 14, color: AppColors.inactive),
                  ),
                ],
              ],
            ),
    ...
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          suffixText: suffix,
          suffixStyle: TextStyle(fontSize: 13, color: done ? AppColors.primary : AppColors.inactive),
...
class _ExercisePickerSheetState extends State<_ExercisePickerSheet> {
  ...
  @override
  Widget build(BuildContext context) {
    final filtered = _presetExercises.where((e) => e.toLowerCase().contains(_query.toLowerCase())).toList();
    ...
    return Container(
      ...
      padding: EdgeInsets.only(bottom: safeBottom + 16, left: 20, right: 20, top: 8),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          // Handle
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: const InputDecoration(
                ...
                prefixIcon: Icon(LucideIcons.search, size: 18, color: AppColors.inactive),
            ...
            child: ListView.separated(
              ...
              itemBuilder: (ctx, i) {
                return ListTile(
                  ...
                  leading: const Icon(LucideIcons.dumbbell, size: 18, color: AppColors.primary),
