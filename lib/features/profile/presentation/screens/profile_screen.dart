...
class ProfileScreen extends ConsumerWidget {
          ...
          child: Column(
            ...
            children: [
              ...
              profileAsync.when(
                ...
                error: (e, st) => const Text("Failed to load profile", style: TextStyle(color: AppColors.danger)),
              ),
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(LucideIcons.userPlus, size: 20, color: AppColors.primary),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(t('referral_tagline'), style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                            const SizedBox(height: 2),
                            Text(t('referral_subtitle'), style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
                          ],
                        ),
                      ),
              ...
              _GroupedListCard(
                rows: [
                  _SettingsRow(icon: LucideIcons.badgeCheck, label: t('personal_details'), onTap: () => context.push('/profile/personal')),
                  _SettingsRow(icon: LucideIcons.settings2, label: t('preferences'), onTap: () => context.push('/profile/preferences')),
                  _SettingsRow(icon: LucideIcons.languages, label: t('language'), onTap: () => context.push('/profile/language')),
                ],
              ),
              ...
              _GroupedListCard(
                rows: [
                  _SettingsRow(icon: LucideIcons.circleDashed, label: t('nutrition_goals'), onTap: () => context.push('/profile/nutrition')),
                ],
              ),
              ...
              _GroupedListCard(
                rows: [
                  _SettingsRow(icon: LucideIcons.logOut, label: 'Sign Out', color: AppColors.primary, onTap: () => _signOut(context)),
                  _SettingsRow(icon: LucideIcons.userMinus, label: 'Delete Account', color: AppColors.danger, onTap: () => _confirmDeleteAccount(context)),
                ],
              ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Text(data['name'] ?? 'User', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                    const SizedBox(width: 6),
                    GestureDetector(
                      onTap: () => _showUpdateProfileBottomSheet(context, ref, data),
                      child: const Icon(LucideIcons.pencil, size: 16, color: AppColors.primary),
                    ),
                  ],
                ),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(sub['plan_name'] ?? 'GojoCalories Pro', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
              Container(
  ...
  void _showUpdateProfileBottomSheet(BuildContext context, WidgetRef ref, Map<String, dynamic> data) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (ctx) => _ProfileUpdateSheet(user: data, onSaved: () => ref.refresh(profileProvider)),
    );
  }
  ...
  Future<void> _confirmDeleteAccount(BuildContext context) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text("This action is permanent and will completely erase all your data and cancel any active subscription. Do you wish to proceed?"),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text("Cancel")),
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text("Delete Permanently", style: TextStyle(color: AppColors.danger))),
        ],
      ),
    );
...
class _ProfileUpdateSheetState extends State<_ProfileUpdateSheet> {
  ...
  Future<void> _save() async {
    ...
    try {
      await ApiClient.instance.put('auth/me/profile', data: {
        'name': _nameCtrl.text,
    ...
    return Container(
      ...
      child: SafeArea(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            const Text("Update Profile", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
            const SizedBox(height: 16),
            ...
            ElevatedButton(
              ...
              child: _loading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white)) : const Text("Save Changes"),
            ),
            const SizedBox(height: 16),
          ],
        ),
...
class _GroupedListCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < rows.length; i++) {
      children.add(rows[i]);
      if (i < rows.length - 1) {
        children.add(const Divider(color: AppColors.border, height: 1, indent: 52));
      }
    }
    return Container(
      decoration: BoxDecoration(
...
class _SettingsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      trailing: color == null ? const Icon(LucideIcons.chevronRight, size: 18, color: AppColors.inactive) : null,
